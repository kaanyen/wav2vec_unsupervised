...
class UnpairedAudioText(FairseqTask):
    ...
    def load_dataset(self, split: str, task_cfg: FairseqDataclass = None, **kwargs):
        ...
        if has_unpaired_text:
            bin_path = os.path.join(self.cfg.text_data, f"{split}.bin")
            if not os.path.exists(bin_path) or os.path.getsize(bin_path) == 0:
                raise RuntimeError(
                    f"Phoneme indexed dataset is empty or missing: {bin_path}. "
                    "Re-run prepare_text after fixing text (non-empty "
                    "data/text/.../lm.phones.filtered.txt). "
                    "Common cause: every word in a line was OOV for the lexicon."
                )
