...
# Optional label/dict files are absent until prepare_text; zsh nomatch would error.
for f in $source_dir/*.wrd(N); do cp "$f" "$tgt_dir/"; done
for f in $source_dir/*.ltr(N); do cp "$f" "$tgt_dir/"; done
for f in $source_dir/*.phn(N); do cp "$f" "$tgt_dir/"; done
for f in $source_dir/dict*(N); do cp "$f" "$tgt_dir/"; done
...
for split in $all_splits; do
  python $FAIRSEQ_ROOT/examples/wav2vec/unsupervised/scripts/apply_pca.py $tgt_dir --split $split --save-dir $tgt_dir/precompute_pca$dim --pca-path $tgt_dir/pca/${dim}_pca --batch-size 32
