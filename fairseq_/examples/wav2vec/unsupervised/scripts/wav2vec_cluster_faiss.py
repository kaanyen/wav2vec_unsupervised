...
# Original code compared device to the string 'cuda', but device is a
# torch.device object, so this was always False and gpu was always False.
# Use explicit CUDA check so k-means uses the GPU only when CUDA is available.
gpu = torch.cuda.is_available()


def _configure_faiss_threads():
    # macOS: multi-threaded Faiss k-means can deadlock with PyTorch/OpenMP (0% CPU).
    # Override with FAISS_OMP_THREADS (e.g. 4) if you know your stack is safe.
    default = "1" if platform.system() == "Darwin" else "0"
    raw = os.environ.get("FAISS_OMP_THREADS", default)
    if raw in ("", "0"):
        return
    try:
        faiss.omp_set_num_threads(int(raw))
