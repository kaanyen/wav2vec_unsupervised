...
# Fairseq preprocess: cap workers on macOS / tiny corpora (workers=70 leaks semaphores).
if [[ -n "${FAIRSEQ_PREPROCESS_WORKERS:-}" ]]; then
  PREP_WORKERS=$FAIRSEQ_PREPROCESS_WORKERS
else
  PREP_WORKERS=$(sysctl -n hw.ncpu 2>/dev/null || echo 4)
  if [[ "$PREP_WORKERS" -gt 8 ]]; then PREP_WORKERS=8; fi
fi
...
# BSD grep has no -P; ERE [0-9]{5,} matches lines with 5+ consecutive digits (same intent as Perl).
cut -f1 -d' ' $target_dir/dict.txt | grep -v -x '[[:punct:]]*' | grep -Ev '[0-9]{5,}' >! $target_dir/words.txt
...
if [ -z "$ESPEAK_PATH" ]; then
  ...
  sed 's/$/ 1/' $target_dir/words.txt | PHONEMIZER_ESPEAK_PATH=$ESPEAK_PATH phonemize -o $target_dir/phones.txt -p ' ' -w '' -l $ph_lg -j $PREP_WORKERS --language-switch remove-flags
  echo "one is ${one}"
...
python $FAIRSEQ_ROOT/fairseq_cli/preprocess.py --dataset-impl mmap --trainpref $target_dir/phones/lm.phones.filtered.txt --workers $PREP_WORKERS --only-source --destdir $target_dir/phones --srcdict $target_dir/phones/dict.phn.txt

$KENLM_ROOT/lmplz -o 4 < $target_dir/lm.upper.lid.txt --discount_fallback --prune 0 0 0 3 >! $target_dir/kenlm.wrd.o40003.arpa
# -s: tolerate ARPA without </s> when corpora are tiny or degenerate (KenLM MissingSentenceMarkerException).
$KENLM_ROOT/build_binary -s $target_dir/kenlm.wrd.o40003.arpa $target_dir/kenlm.wrd.o40003.bin
