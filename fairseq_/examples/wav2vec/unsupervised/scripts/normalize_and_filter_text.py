...
def load_fasttext_model(path):
    """Load LID model; avoid importing fasttext at module load (broken wheels on macOS)."""
    try:
        import fasttext as ft
    except ImportError as err:
        print(
            f"fasttext could not be imported ({err}). Proceeding without language filtering. "
            f"Reinstall from source: pip install --no-binary fasttext fasttext",
            file=sys.stderr,
        )
        return None
    try:
        ...
        print(f"failed to load fasttext model {path}: {err}", file=sys.stderr)
        return None
