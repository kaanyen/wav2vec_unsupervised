...
try:
    ...
    warnings.warn(
        "flashlight python bindings are not installed; W2lViterbiDecoder will use a "
        "PyTorch greedy CTC fallback. For KenLM / lexicon decoding, install flashlight from "
        "https://github.com/facebookresearch/flashlight/tree/master/bindings/python"
    )
    HAS_FLASHLIGHT = False
    CpuViterbiPath = None  # type: ignore
    get_data_ptr_as_bytes = None  # type: ignore

    class CriterionType:  # type: ignore
        CTC = 0
...
def _decoder_kenlm_bin_path(args) -> str:
    """Path to KenLM binary for W2lKenLMDecoder / LexiconFree KenLM branch.

    ASR scripts use ``kenlm_model``; wav2vec unsupervised ``w2vu_generate`` uses
    ``lm_model`` only. Prefer ``kenlm_model`` when set.
    """
    p = getattr(args, "kenlm_model", None)
    if p:
        return p
    p = getattr(args, "lm_model", None)
    if not p:
        raise ValueError("KenLM decode requires kenlm_model or lm_model (path to .bin)")
    return p
...
class W2lViterbiDecoder(W2lDecoder):
    ...
    def decode(self, emissions):
        ...
        if not HAS_FLASHLIGHT:
            # Greedy frame-wise argmax + CTC collapse (same post-process as Flashlight path).
...
class W2lKenLMDecoder(W2lDecoder):
    def __init__(self, args, tgt_dict):
        ...
        if args.lexicon:
            ...
            km_path = _decoder_kenlm_bin_path(args)
            self.lm = KenLM(km_path, self.word_dict)
            ...
            self.lm = KenLM(_decoder_kenlm_bin_path(args), self.word_dict)
...
class W2lFairseqLMDecoder(W2lDecoder):
    def __init__(self, args, tgt_dict):
        ...
        km_path = _decoder_kenlm_bin_path(args)
        checkpoint = torch.load(km_path, map_location="cpu")
        ...
        with open_dict(lm_args.task):
            lm_args.task.data = osp.dirname(km_path)
